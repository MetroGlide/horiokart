...
## Yaw を含めた最適化 (Orientation-aware optimization)

この節では、既存の 2D (x,y) 最適化に yaw (θ) を含める設計と実装ガイドを示す。

目的
- ノード状態を位置のみから位置＋方位 (x,y,θ) に拡張し、ループクロージャやエッジの回転情報を最適化に反映することで、地図の角度的一貫性を改善する。

変数化
- 各ノード i の最適化変数を (x_i, y_i, θ_i) とする。変数ベクトルは z = [x0,y0,θ0, x1,y1,θ1, ...] になる。

残差設計
- GNSS 制約: GNSS は通常位置のみの観測であるため、位置成分のみの 2 要素残差 r_g = (x_i,y_i) - (x_gnss,y_gnss) を生成する。もし GNSS 由来で角度情報がある場合は角度残差を追加できる。
- エッジ制約 (odometry/loop): 3 要素残差 r_e = [Δx_in_a_frame - dx_meas, Δy_in_a_frame - dy_meas, wrap(dθ_pred - dθ_meas)] を用いる。
  - ここで Δ_pred = R(θ_a)^T (p_b - p_a)、dθ_pred = wrap(θ_b - θ_a)。角度は常に -π..π に wrap する。

共分散と情報行列
- エッジが位置＋角度の 3x3 共分散を提供する場合、Cov_3x3 を正則化 (cov_regularization) して Info = inv(Cov_3x3 + λ I) を作る。sqrt-information を残差に掛けて最尤的に扱う。
- GNSS が位置共分散 (2x2) のみを提供する場合は位置ブロックに対して sqrt-info を用い、角度はエッジ情報に依存させる。

ヤコビアンと実装の注意点
- 初期段階では数値ヤコビアンで実装して動作確認するのが簡単。高速化・安定化が必要なら解析ヤコビアンを実装する。
- 角度関連の偏微分の概念:
  - ∂Δ/∂x_a = -R(θ_a)^T,  ∂Δ/∂x_b = +R(θ_a)^T
  - ∂Δ/∂θ_a = -R(θ_a)^T * J * (p_b - p_a)  （J は 2x2 の回転微分）
  - 角度残差: ∂dθ/∂θ_a = -1, ∂dθ/∂θ_b = +1

ゲージ固定（自由度の固定）
- グローバルな並進 + 回転自由度を解消するため、少なくとも 1 ノードの (x,y,θ) を固定するか、強い仮想観測（初期値に対する大きな情報行列）を追加して固定する。実装の簡便さからは仮想観測を推奨する。

スケーリングとロバスト性
- 角度はラジアンで扱う。位置と角度の寄与バランスのため `--angle-weight` のようなスケール調整パラメータを用意すると良い。
- ロバスト損失（Huber 等）は位置・角度両方に適用可能。

CLI フラグ（提案）
- `--optimize-include-yaw` : yaw を最適化に含める (default: False)
- `--fix-first-node` : 最初のノードを固定する（virtual obs による実装）(default: True)
- `--angle-weight` : 角度残差のスケーリング (default: 1.0, 単位 rad)
- `--cov-regularization-angle` : 角度ブロックの正則化値 (default: 1e-6)

ユニットテスト
- 最小テストケース: 3 ノードの直線チェーンで隣接エッジに dθ=0 を設定し、GNSS で位置を拘束。初期 yaw に小さなノイズを入れて最適化で yaw が収束することを確認する。

実装手順（最小 PoC）
1. `tools/gnss_optimize.py` の変数長を 2N→3N に変更、index helper を追加
2. edge residual を 3 要素化（pos + angle）し、角度差は wrap で正規化する
3. GNSS 残差は位置のみ生成（角度情報がある場合は追加）
4. 最初のノードを仮想観測で固定する（情報行列に大きな値を使う）
5. 数値ヤコビアンで動作確認、必要なら解析ヤコビを追加

注意点
- 角度の wrap 処理忘れと角度のスケーリングミスが典型的な落とし穴。特に大回転やループがある場合はロバスト性に注意する。
