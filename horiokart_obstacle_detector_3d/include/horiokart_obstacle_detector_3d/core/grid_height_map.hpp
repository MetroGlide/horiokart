...
public:
  ...
  GridHeightMap(double x_min, double x_max, double y_min, double y_max,
                double cell_size);
  ...
  void setParameters(int min_obs_per_cell_for_confident_median,
                     int radius_interp_cells, double interp_power_p,
                     double interp_alpha, double max_interp_area_m2,
  ...
  // セル (ix,iy) の局所傾斜（度）を返す。範囲外または観測なしの場合は false
  // を返す
