...
public:
  ...
  // Parameters for interpolation / temporal fusion / timeout
  void setParameters(
  ...
  // return local slope in degrees for cell (ix,iy); returns false if out of range or no observation
...
private:
  // interpolation and temporal fusion params
  ...
  double observation_timeout_ = 0.5;  // seconds
  ...
  std::vector<std::vector<std::vector<double>>> cell_buffers_;  // per-cell z list
  std::vector<GridCell> cells_;                                 // row-major: ix * cols_ + iy
  // per-cell local slope in degrees (computed from height map)
