    ...
    get_param_double_vec)
{
  ...
  for (; itx != itx.end(); ++itx, ++ity, ++itz) {
    ...
    if (
      p.x < roi_x_min || p.x > roi_x_max || p.y < roi_y_min || p.y > roi_y_max || p.z < roi_z_min ||
      p.z > roi_z_max) {
      if (has_rgb_field) ++(*it_rgb);
      if (has_intensity_field) ++(*it_intensity);
      continue;
    ...
    std::vector<double> inorm =
      get_param_double_vec("intensity_normalize_range", std::vector<double>{0.0, 1.0});
    ...
    if (ci.has_intensity) {
      ...
      in = obstacle_detector::compensateIntensity(
        in, p, sensor_origin, sensor_fwd, intensity_distance_ref, intensity_distance_power,
        intensity_compensate_distance, intensity_compensate_angle, intensity_angle_min_dot);
