...
CloudProcessorResult CloudProcessor::process(
  ...
  // 1) グリッドを構築
  res.grid = std::make_unique<GridHeightMap>(cfg_.roi_x_min, cfg_.roi_x_max,
  ...
  if (cfg_.slope_method == "pca") {
    res.pca_results = obstacle_detector::computePcaSlopesAndNormals(
        pts, *res.grid, cfg_.pca_radius_m, cfg_.pca_min_points,
        cfg_.grid_cell_size, cfg_.roi_x_min, cfg_.roi_y_min);
  }

  // 3) ground_separator を使って点を地面 / 非地面に分類
  for (const auto &p : pts) {
    int ix = static_cast<int>(
        std::floor((p.x - cfg_.roi_x_min) / cfg_.grid_cell_size));
    int iy = static_cast<int>(
        std::floor((p.y - cfg_.roi_y_min) / cfg_.grid_cell_size));
    GridCell cell;
    ...
    if (res.grid->getCell(ix, iy, cell) && cell.has_observation) {
      auto itmeta =
          point_meta.find(std::to_string(p.x) + "," + std::to_string(p.y) +
                          "," + std::to_string(p.z));
      if (itmeta != point_meta.end() && itmeta->second.has_intensity) {
        double in = itmeta->second.intensity;
        cell.confidence =
            std::min(1.0, cell.confidence * (1.0 - 0.2) + in * 0.2);
      }
      double dz = std::abs(p.z - cell.height_median);
      double score = ground_separator.computeGroundScore(
          slope_deg, cell.height_variance, cell.confidence);
      ...
      bool is_ground_by_score = (cell.ground_ema >= high_th) ||
                                (cell.ground_ema > low_th && cell.is_ground);
  ...
  if (cfg_.dynamic_leaf && pts.size() > 0) {
    double scale = std::sqrt(static_cast<double>(pts.size()) /
                             static_cast<double>(cfg_.target_points));
  ...
  for (const auto &c : res.clusters) {
    ...
    if (height < cfg_.min_obstacle_height)
      continue;
    if (c.volume < cfg_.min_obstacle_volume)
      continue;
    res.obstacle_points.insert(res.obstacle_points.end(), c.points.begin(),
                               c.points.end());
  }
