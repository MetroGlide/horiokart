...
std::vector<Cluster> ClusterDetector::extractClusters(const std::vector<PointXYZ> & points) const
{
  ...
  for (const auto & p : points) {
    cloud->push_back(
      pcl::PointXYZ(static_cast<float>(p.x), static_cast<float>(p.y), static_cast<float>(p.z)));
  }

  // optional VoxelGrid downsampling
  pcl::PointCloud<pcl::PointXYZ>::Ptr process_cloud = cloud;
  // keep an index mapping from process_cloud -> original cloud when downsampling
  // if non-empty, voxel_index_map[i] contains indices into 'cloud' that map to
  // process_cloud->points[i]
  std::vector<std::vector<int>> voxel_index_map;
  if (voxel_leaf_size_ > 0.0) {
    // If no further preprocessing is requested, build our own voxelization so we can preserve
    // mapping
    if (!passthrough_z_enable_ && outlier_radius_ <= 0.0) {
      ...
      // fallback to pcl::VoxelGrid when other preprocessing is enabled (no preserved mapping)
      pcl::PointCloud<pcl::PointXYZ>::Ptr filtered(new pcl::PointCloud<pcl::PointXYZ>);
  ...
  int id = 0;
  // If expand_to_original_cloud_ is set, we will expand cluster indices to original points by
  ...
  for (const auto & indices : cluster_indices) {
    ...
    if (expand_to_original_cloud_) {
      // If we constructed a voxel_index_map during downsampling, use it for exact expansion
        ...
        for (int idx : indices.indices) {
          if (idx >= 0 && static_cast<size_t>(idx) < voxel_index_map.size()) {
            for (int oi : voxel_index_map[idx]) {
              ...
              PointXYZ op{opt.x, opt.y, opt.z};
              c.points.push_back(op);
              c.centroid.x += op.x;
              c.centroid.y += op.y;
              c.centroid.z += op.z;
              xmin = std::min(xmin, (double)op.x);
              ymin = std::min(ymin, (double)op.y);
              zmin = std::min(zmin, (double)op.z);
              xmax = std::max(xmax, (double)op.x);
              ymax = std::max(ymax, (double)op.y);
              zmax = std::max(zmax, (double)op.z);
            }
          }
        }
      } else {
    ...
    // filter by size
    if ((int)n < min_cluster_size_ || (int)n > max_cluster_size_) {
      continue;
    }
