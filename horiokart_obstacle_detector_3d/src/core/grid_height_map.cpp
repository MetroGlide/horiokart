...
GridHeightMap::GridHeightMap(double x_min, double x_max, double y_min,
                             double y_max, double cell_size)
...
void GridHeightMap::finalizeFrame(double now_sec) {
  ...
  for (int i = 0; i < rows_; ++i) {
    for (int j = 0; j < cols_; ++j) {
      ...
      bool prev_has_observation =
          cell.has_observation && (cell.last_observed_time > 0.0);
      ...
      if (prev_has_observation) {
        ...
        cell.height_median = temporal_alpha_height_ * new_height +
                             (1.0 - temporal_alpha_height_) * prev_height;
        cell.confidence = temporal_alpha_conf_ * new_conf +
                          (1.0 - temporal_alpha_conf_) * prev_conf;
      } else {
        // first observation for this cell in recent history: take new values
        // directly
  ...
  for (int i = 1; i < rows_ - 1; ++i) {
    for (int j = 1; j < cols_ - 1; ++j) {
      ...
      double h_x1 = cells_[index(i + 1, j)].has_observation
                        ? cells_[index(i + 1, j)].height_median
                        : c.height_median;
      double h_x0 = cells_[index(i - 1, j)].has_observation
                        ? cells_[index(i - 1, j)].height_median
                        : c.height_median;
      double h_y1 = cells_[index(i, j + 1)].has_observation
                        ? cells_[index(i, j + 1)].height_median
                        : c.height_median;
      double h_y0 = cells_[index(i, j - 1)].has_observation
                        ? cells_[index(i, j - 1)].height_median
                        : c.height_median;
  ...
  for (int i = 0; i < rows_; ++i) {
    for (int j = 0; j < cols_; ++j) {
      ...
      if (area_m2 <= max_interp_area_m2_) {
        // interpolate each cell in stack using IDW from neighbors within
        // radius_interp_cells_
        for (const auto &pr : stack) {
          ...
          for (int di = -radius_interp_cells_; di <= radius_interp_cells_;
               ++di) {
            for (int dj = -radius_interp_cells_; dj <= radius_interp_cells_;
                 ++dj) {
              int ni = ci + di;
              int nj = cj + dj;
              if (ni < 0 || ni >= rows_ || nj < 0 || nj >= cols_) {
              ...
              double w =
                  ncell.confidence / (std::pow(dist + 1e-6, interp_power_p_));
...
void GridHeightMap::setParameters(int min_obs_per_cell_for_confident_median,
                                  int radius_interp_cells,
                                  double interp_power_p, double interp_alpha,
                                  double max_interp_area_m2,
                                  double temporal_alpha_height,
                                  double temporal_alpha_conf,
                                  double observation_timeout) {
  min_obs_per_cell_for_confident_median_ =
      min_obs_per_cell_for_confident_median;
...
bool GridHeightMap::getCellSlopeDeg(int ix, int iy,
                                    double &out_slope_deg) const {
  if (ix < 0 || ix >= rows_ || iy < 0 || iy >= cols_) {
    return false;
  }
  std::lock_guard<std::mutex> lk(mutex_);
