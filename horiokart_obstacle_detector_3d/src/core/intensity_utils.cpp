...
double obstacle_detector::compensateIntensity(
    double intensity, const PointXYZ &point,
    const Eigen::Vector3d &sensor_origin, const Eigen::Vector3d &sensor_forward,
    double distance_ref, double distance_power, bool compensate_distance,
    bool compensate_angle, double angle_min_dot) {
  double in = intensity;
  if (compensate_distance) {
    double dist = std::hypot(point.x, point.y);
    double factor =
        std::pow((dist / std::max(1e-6, distance_ref)), distance_power);
    if (factor > 0.0) {
      in = in * std::min(10.0, 1.0 / factor);
    }
  }
  if (compensate_angle) {
    Eigen::Vector3d ray(point.x - sensor_origin.x(),
