...
TEST(IntensityUtils, DistanceOnly) {
  ...
  double in = 1.0;
  double out =
      compensateIntensity(in, p, origin, fwd, 1.0, 2.0, true, false, 0.2);
  // at distance 1 and ref 1, factor==1 => out ~ in
  EXPECT_NEAR(out, 1.0, 1e-6);
  PointXYZ p2{2.0f, 0.0f, 0.0f};
  double out2 =
      compensateIntensity(in, p2, origin, fwd, 1.0, 2.0, true, false, 0.2);
...
TEST(IntensityUtils, AngleOnly) {
  ...
  double in = 1.0;
  double out =
      compensateIntensity(in, p, origin, fwd, 1.0, 2.0, false, true, 0.5);
  // dot = 0 -> scale to 0
